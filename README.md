...
| Class | Purpose |
|-------|---------|
| `KioskController` | Main kiosk interface implementation |
| `PricingStrategy` | Interface for dynamic pricing policies |
| `Command` | Interface for executable/undoable operations |
| `InventorySnapshot` | Memento for capturing inventory state |
| `PersistenceService` | Gson-based JSON serialization |
| `CentralRegistry` | Singleton for system configuration |

## Test Coverage

- **PricingStrategyTest**: Validates all three pricing strategies and runtime switching
- **CommandRollbackTest**: Verifies atomic transactions and rollback on failure
